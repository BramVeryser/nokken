...
if handles.temp.motionlaw==1 % dwell
		assert(L1-L0 == 0, 'The given input does represent a dwell');
		S=L0*ones(1,size(x,2));
		V=0*ones(1,size(x,2));
		A=0*ones(1,size(x,2));
        
elseif handles.temp.motionlaw==2 % minimal rms acceleration
		L=L1-L0;
		S=L*(3*x.^2-2*x.^3)+L0;
		V=L/beta*(6*x-6*x.^2);
		A=L/beta^2*(6-12*x);
        
elseif handles.temp.motionlaw==3 % harmonische
		L=L1-L0;
		S=L*(1-cos(pi*x))/2+L0;
		V=L/beta*sin(pi*x)*pi/2;
		A=L/beta^2*cos(pi*x)*pi^2/2;
        
elseif handles.temp.motionlaw==4 % volle cycloide
		L=L1-L0;
		S=L*(x-sin(2*pi*x)/2/pi)+L0;
		V=L/beta*(1-cos(2*pi*x));
		A=2*pi*L/beta^2*(sin(2*pi*x));
	
elseif handles.temp.motionlaw==5 % 5th degree poly
		L=L1-L0;
        S = L0+L*(6*x.^5-15*x.^4+10*x.^3);
		V= L/beta*(30*x.^4-60*x.^3+30*x.^2);
		A= L/beta^2*(120*x.^3-180*x.^2+60*x);

elseif handles.temp.motionlaw==6 % 7th degree poly
		L=L1-L0;
...
function motionlaw_Callback(hObject, eventdata, handles)
handles.temp.motionlaw =  get(hObject,'Value');
guidata(hObject, handles);
...
function motionlaw_CreateFcn(hObject, eventdata, handles)
...
appdata = [];
appdata.lastValidTag = 'motionlaw';
...
'Min',get(0,'defaultuicontrolMin'),...
'SliderStep',get(0,'defaultuicontrolSliderStep'),...
'String',{  'dwell'; 'min rms acc'; 'harmonic'; 'cycloid'; '5th degree poly'; '7th degree poly'},...
'Style','popupmenu',...
'Value',1,...
'Position',[0.01 0.54 0.245 0.18],...
'BackgroundColor',[1 1 1],...
'Callback',@(hObject,eventdata)matcam('motionlaw_Callback',hObject,eventdata,guidata(hObject)),...
'Children',[],...
'ForegroundColor',get(0,'defaultuicontrolForegroundColor'),...
'Enable',get(0,'defaultuicontrolEnable'),...
'TooltipString',blanks(0),...
'Visible',get(0,'defaultuicontrolVisible'),...
'ButtonDownFcn',blanks(0),...
'CreateFcn', {@local_CreateFcn, @(hObject,eventdata)matcam('motionlaw_CreateFcn',hObject,eventdata,guidata(hObject)), appdata} ,...
'DeleteFcn',blanks(0),...
'Tag','motionlaw',...
'UserData',[],...
'KeyPressFcn',blanks(0),...
